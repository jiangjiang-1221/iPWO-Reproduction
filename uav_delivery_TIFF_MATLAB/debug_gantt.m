...
load(matpath);
fprintf('=== routes / cust_seq structure ===\n');
for u=1:env.Nu
...
if isfield(best_diag,'arr')
    ...
    fprintf('best_diag has no arr field\n');
end
fprintf('=== recompute route_timing check ===\n');
for u=1:env.Nu
...
for idx = 2:size(nodes, 1)
    ...
    t = t + sl/env.V;
    if idx >= 3 && idx <= 2 + nC          % cruise-and-hover delivery point above customer
        hi = hi + 1;
        j = seq(hi);
