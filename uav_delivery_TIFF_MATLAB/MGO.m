...
function [best_cost, best_M, Convergence_curve] = MGO(SearchAgents_no, MaxFEs, lb, ub, dim, fobj)
    %% Initialization
    FEs = 0;
    best_cost = inf; % Change to -inf for maximization problems
    best_M = zeros(1, dim);
    M = MGO_initialization(SearchAgents_no, dim, ub, lb); % Initialize the set of random solutions
    costs = zeros(1, SearchAgents_no);
    for i = 1:SearchAgents_no
        costs(i) = fobj(M(i,:));
        FEs = FEs + 1; % Increment FEs on each evaluation
    ...
    while FEs < MaxFEs
        ...
        for i = 1:SearchAgents_no    
            ...
            FEs = FEs + 1;  % Increment FEs on each fitness evaluation
