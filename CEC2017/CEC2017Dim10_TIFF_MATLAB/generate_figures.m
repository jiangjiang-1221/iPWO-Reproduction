...
% =====================================================================
%  generate_figures.m
%  Reproduce CEC2017 (Dim = 10) Boxplot and Convergence figures as TIFF.
%
...
%                                  (data-driven: 0 .. nfe-1)
%   - Boxplot
%       * Title shows the CEC2017 function -> "CEC2017 F<id>, Dim=10"
%       * Y-axis uses scientific (e) notation
...
%   The 58 .tif figures are written to the 'figures' subfolder.
%
...
% ---- discover functions from the Best_Values files ----
...
for k = 1:numel(xlsx_files)
    tk = regexp(xlsx_files(k).name, 'F(\d+)_Dim10', 'tokens');
    func_ids(k) = str2double(tk{1}{1});
end
func_ids = sort(func_ids);

fprintf('Generating figures for %d functions (18 pt font, %d DPI, gradient boxes)...\n', ...
        numel(func_ids), DPI);
...
for fi = 1:numel(func_ids)
    ...
    bw = fullfile(data_dir, sprintf('CEC2017_F%d_Dim10_Best_Values.xlsx', fid));
    ...
    title(ax, sprintf('CEC2017 F%d, Dim=10', fid), ...
          'FontSize', FONT_SIZE, 'FontWeight', 'bold', 'FontName', 'Times New Roman');
    ...
    out_b = fullfile(out_dir, sprintf('CEC2017_F%d_Dim10_Boxplot_BestValues.tif', fid));
    exportgraphics(gcf, out_b, 'Resolution', DPI, 'ContentType', 'image');
    close;

    % ============== CONVERGENCE (no inset, x starts at 0) ==============
    cw = fullfile(data_dir, sprintf('CEC2017_F%d_Dim10_Avg_Convergence.xlsx', fid));
    ...
    out_c = fullfile(out_dir, sprintf('CEC2017_F%d_Dim10_Convergence.tif', fid));
...
fprintf('All %d figures written to: %s\n', numel(func_ids), out_dir);
