iPWO CEC2017 D=100 性能评估与可视化 —— 可复现包
================================================

一、内容结构
  code/     MATLAB 可复现代码
    main30a.m                    完整测试入口: 运行 iPWO (MaxFEs=10000*D, 30次独立运行,
                                 D=100, F1+F3~F30 共29函数), 并输出原始结果与复现数据
  ...
  data/
    raw/                 29 个原始结果文件 CEC2017_F*_Dim100_iPWO_Results.mat
    ...
    iPWO_Dim100_Summary.csv  29 函数最优成绩与平均曲线终值汇总

  figures/              单独面板 TIFF (2.928cm 见方, 600 DPI, 692x692 px, Times New Roman)
    F*_Dim100_1_3D_Landscape.tif ... F*_Dim100_5_Search_History.tif   各面板子图 (145张)
...
二、如何复现
  1) 完整复现(从优化开始, 约4-5小时):
       cd code; main30a
     - 按 CEC2017 标准预算 MaxFEs=10000*D 运行 iPWO, 每函数30次, D=100
