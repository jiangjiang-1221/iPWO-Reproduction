function [best_fitness, best_solution, Convergence_curve, history] = iPWO_history(N, maxFES, lb, ub, dim, fobj)
...
%             around the elite in the late stage.
    ...
    history.popPos = cell(1, T);
    history.popFit = cell(1, T);
    history.bestPosIter = zeros(T, dim);
    ...
    while FES + NP <= maxFES && iter < T
        ...
        history.popPos{iter} = X;
        history.popFit{iter} = f;
        history.bestPosIter(iter, :) = best_solution;
